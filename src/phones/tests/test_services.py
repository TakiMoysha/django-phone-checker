...
from phones.domain.services import (
    bulk_download_registry_files,
    load_registry_from_file,
)
from phones.helpers import get_defphone_repository
...
class TestServices(unittest.IsolatedAsyncioTestCase):
    ...
    @unittest.skipIf(
        os.getenv("TEST_WITH_DATABASE") not in TRUE_VALUES,
        "required postgres",
    )
    async def test_load_def_to_database(self) -> None:
        _ = await load_registry_from_file(
            self.test_file,
            repository=get_defphone_repository(),
            from_scratch=True,
        )
